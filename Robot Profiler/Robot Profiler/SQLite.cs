...
namespace Robot_Profiler
{
    class SQLite
    {
        ...
        public List<String> SelectQueryDurations(String table, String Name)
        {
            ...
            using (SQLiteConnection sqlite = new SQLiteConnection("Data Source=" + sqlfilename + ";Version=3;Pooling=True;Max Pool Size=10;Synchronous=off;FailIfMissing=True;Journal Mode=Off;"))
            {
                using (SQLiteCommand query = new SQLiteCommand("SELECT Name,Duration FROM " + table + " WHERE Name=@Name", sqlite))
                {
                    try
                    {
        ...
        public void CreateTable(String tablename)
        {
            ...
            using (SQLiteConnection sqlite = new SQLiteConnection("Data Source=" + sqlfilename + ";Version=3;Pooling=True;Max Pool Size=10;Synchronous=off;FailIfMissing=True;Journal Mode=Off;"))
            {
                using (SQLiteCommand createTable = new SQLiteCommand("CREATE TABLE " + TableName + " ( `Type` TEXT NOT NULL, `Name` TEXT NOT NULL, `Status` TEXT NOT NULL, `StartTime` TEXT, `EndTime` TEXT, `Duration` TEXT, `ID` INTEGER, `ParentID` INTEGER, PRIMARY KEY(ID))", sqlite))
                {
                    try
                    {
                        sqlite.Open();
        ...
        public void InsertElement(RobotElement elem)
        {
            using (SQLiteConnection sqlite = new SQLiteConnection("Data Source=" + sqlfilename + ";Version=3;Pooling=True;Max Pool Size=10;Synchronous=off;FailIfMissing=True;Journal Mode=Off;"))
            {
                using (SQLiteCommand insertSQL = new SQLiteCommand("INSERT INTO " + TableName + " (Type, Name, Status, StartTime, EndTime, Duration, ID, ParentID) VALUES (@Type, @Name, @Status, @StartTime, @EndTime, @Duration, @ID, @ParentID)", sqlite))
                {
        ...
        public void UpdateElement(RobotElement elem)
        {
            using (SQLiteConnection sqlite = new SQLiteConnection("Data Source=" + sqlfilename + ";Version=3;Pooling=True;Max Pool Size=10;Synchronous=off;FailIfMissing=True;Journal Mode=Off;"))
            {
                using (SQLiteCommand updateSQL = new SQLiteCommand("UPDATE " + TableName + " SET Type=@Type, Name=@Name, Status=@Status, StartTime=@StartTime, EndTime=@EndTime, Duration=@Duration, ParentID=@ParentID WHERE ID=@ID;", sqlite))
                {
        ...
        public void DeleteElement(RobotElement elem)
        {
            using (SQLiteConnection sqlite = new SQLiteConnection("Data Source=" + sqlfilename + ";Version=3;Pooling=True;Max Pool Size=10;Synchronous=off;FailIfMissing=True;Journal Mode=Off;"))
            {
                using (SQLiteCommand deleteSQL = new SQLiteCommand("DELETE FROM " + TableName + " WHERE ID=@ID ;", sqlite))
                {
